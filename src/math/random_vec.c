...
/*   Updated: 2025/10/01 12:31:47 by apregitz         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_vec3	random_unit_vec3(void)
{
	...
	while (true)
	{
		...
		if (lensq > 1e-160 && lensq <= 1)
			return (vec3_divide_inline(&vec, sqrt(lensq)));
	}
}
...
t_vec3	random_on_hemisphere(t_vec3 *normal)
{
	...
	if (vec3_dot_inline(&on_unit_sphere, normal) > 0.0)
		return (on_unit_sphere);
	return (vec3_overload_inline(&on_unit_sphere));
}
